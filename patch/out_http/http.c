                     ...
                     const char *tag, int tag_len)
{
    ...
    // flb_plg_info(ctx->ins, "Begin http_post...");
    ...
    // flb_plg_info(ctx->ins, "append headers...");
    /* Append headers */
    ...
    // flb_plg_info(ctx->ins, "Apply config map...");
    ...
    // flb_plg_info(ctx->ins, "http do to %s:%i",
    //              ctx->host, ctx->port);
    ...
    // flb_plg_info(ctx->ins, "return http_post...");
    return out_ret;
}
...
static int sequential_json(struct flb_out_http *ctx,
                     const char *data, uint64_t bytes,
                     ...
                     int tag_len)
{
    ...
    // flb_plg_info(ctx->ins, "IN sequential_json #records=%i", records);
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off) == ok) {
        ...
        flb_plg_info(ctx->ins, "single record -> output json: %s", out_js);
        ...
        // json_str = flb_msgpack_to_json_str(json_str_len, obj);
    }


    msgpack_unpacked_destroy(&result);
    // flb_plg_info(ctx->ins, "returning from sequential_json");
    return ret;
}
                          ...
                          struct flb_config *config)
{
    ...
    if ((ctx->out_format == FLB_PACK_JSON_FORMAT_JSON) ||
        ...
        //                                        ctx->date_key);
        // if (json != NULL) {
        ret = sequential_json(ctx, data, bytes, 
                              ctx->out_format,
                              ctx->json_date_format,
                              ctx->date_key, tag,
                              tag_len);
        // flb_plg_info(ctx->ins, "IN cb_http_flush -> ret sequectial json:%i", ret);
        // if (ret == FLB_OK) {        
        // }
        ...
        flb_plg_error(ctx->ins, "IN cb_http_flush -> out gelf");
        ret = http_gelf(ctx, data, bytes, tag, tag_len);
    }
    else {
        flb_plg_error(ctx->ins, "IN cb_http_flush -> plain data post");
